...
async function captureCloudProfile() {
  const rand = Math.floor(Math.random() * 1000000);
  const cloudUname = `pennypops123_${rand}`;
  ...
  const cloudId = await registerUser(cloudUname, 'pennypops123', `pennypops_${rand}@arena.test`);
  ...
  await pool.query(
    `UPDATE users SET ui_theme = 'cloud' WHERE id = $1`, [cloudId]
  );
  ...
  await cardElement.screenshot({ path: '/home/user/cloud_profile_rendered.png' });
  await page.screenshot({ path: '/home/user/cloud_modal_full_screen.png' });

  await browser.close();
  console.log('Screenshots saved to /home/user/cloud_profile_rendered.png and /home/user/cloud_modal_full_screen.png');
  process.exit(0);
}

captureCloudProfile().catch((err) => {
  console.error(err);
  process.exit(1);
});
