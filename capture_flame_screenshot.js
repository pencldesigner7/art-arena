...
async function captureFlameScreenshot() {
  console.log('Capturing Flame Theme profile modal screenshot...');
  ...
  // Set subscription and flame theme
  ...
  await pool.query(
    `UPDATE users SET ui_theme = 'flame', created_at = '2026-09-02T12:00:00Z' WHERE id = $1`,
    [userId]
  );
  ...
  // Set user stats to 14 battles, 0 wins, 0 losses, 14 draws
  // We can insert / update battles or we can test as a friend viewing pennypops123
  // Let's create a viewer user who is friends with pennypops123 so the button says "Remove Friend"
  ...
  if (cardElement) {
    await cardElement.screenshot({
      path: '/home/user/flame_theme_rendered.png',
    });
    console.log('✓ Saved screenshot to /home/user/flame_theme_rendered.png');
  }
...
captureFlameScreenshot().catch((err) => {
